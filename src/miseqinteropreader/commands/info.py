...
def execute(args: argparse.Namespace) -> int:
    ...
    print(f"Metrics available: {len(available_metrics)}/{len(MetricFile)}")
    ...
    if lanes:
        print(f"Lanes: {len(lanes)} (range: {min(lanes)}-{max(lanes)})")
    if tiles:
        print(f"Tiles: {len(tiles)}")
    if cycles:
        print(f"Cycles: {len(cycles)} (max: {max(cycles)})")
    ...
    if args.verbose:
        ...
        for metric in available_metrics:
            try:
                ...
                print(f"  • {metric.name}: {metric_file.name} ({file_size:,} bytes)")
            except FileNotFoundError:
                pass
